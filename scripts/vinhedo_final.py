...
# -*- coding: utf-8 -*-
"""
Recorte final do vinhedo a partir de uma COORDENADA informada (ponto + buffer).
Gera curvas de nível (5 m), declividade e mapas, recortados na janela.

Coordenada informada: lat -30.5439, lon -52.5219 (Encruzilhada do Sul / RS).
Ajuste CLAT/CLON/HALF abaixo para outro ponto, ou use scripts/gerar_curvas_nivel.py.
"""
...
CLAT, CLON = -30.5439, -52.5219      # coordenada informada (lat, lon)
HALF = 1750                           # metros -> janela de 3,5 km
NOME = "vinhedo_lidio_carraro"
...
zmean = np.nanmean(demv); z_pt = demv[demv.shape[0]//2, demv.shape[1]//2]
...
print(f"coord ({CLAT},{CLON}) -> UTM ({cx:.0f},{cy:.0f}) | alt.ponto~{z_pt:.0f} m | alt.media={zmean:.0f} m | decliv.media={smean:.1f}%")
...
gpd.GeoDataFrame({"nome":["Vinhedo Lidio Carraro (coordenada informada)"],
                  "lat":[CLAT],"lon":[CLON],
                  "alt_media_m":[round(zmean)],"decliv_media_pct":[round(smean,1)]},
                 geometry=[bb_wgs], crs="EPSG:4326").to_file(
    os.path.join(ROOT,"dados",f"{NOME}_bbox.geojson"), driver="GeoJSON")
...
ax.annotate(f"{CLAT}, {CLON}",(cx,cy),color="navy",fontsize=9,xytext=(8,8),textcoords="offset points")
ax.set_title("Vinhedo Lidio Carraro — Encruzilhada do Sul/RS (coordenada informada)\n"
...
ax.set_title("Declividade — Vinhedo Lidio Carraro (coordenada informada) Encruzilhada do Sul/RS\nClasses Embrapa | Copernicus GLO-30 | UTM 22S",fontsize=9)
